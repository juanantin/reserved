...
//
// Redeem tx (a holder burns RHLD; the token contract pays out four bStocks — CRCLB,
...
const en: DocsSection[] = [
  ...
  {
    id: "token",
    title: "The token (RHLD)",
    blocks: [
      p(
        "RHLD's entire supply — 1,000,000,000 — was minted exactly once, directly into the PancakeSwap V3 pool, as part of the single launch transaction. No RHLD has been minted before or since that transaction."
      ),
      ul([
        ...
        "Calling burn(uint256) on your own balance does more than remove RHLD from supply: the contract pays out a pro-rata share of every bStock the treasury holds, to you, in that same transaction — confirmed by a real, executed burn (tx 0xaa57e40d…ead9fb). See Treasury below for what that transaction actually shows.",
      ]),
    ],
  },
  {
    ...
    blocks: [
      p(
        "A typical launch mints a position through PancakeSwap's NonfungiblePositionManager, which hands back an NFT. Whoever holds that NFT can remove the liquidity, and an NFT can be transferred, lost, or stolen like any other asset. RHLD skipped that path: the token contract called the pool's mint() function directly, naming itself as the position's owner rather than routing through a position manager. This is confirmed directly by the pool's own Mint event in the launch transaction, which names owner as the RHLD token contract's address — not a wallet, not a locker contract, not an NFT."
      ),
      p(
        "The position was seeded single-sided — RHLD only, no BNB — from just above the launch price up to the maximum tick PancakeSwap V3 allows. BNB coming in from buys accumulates as trading fees rather than sitting idle in the position."
      ),
      p(
  ...
  },
  {
    ...
    blocks: [
      ...
      p(
        "Redemption is confirmed live. In tx 0xaa57e40d…ead9fb, a holder called burn(185,458.208224…) on their own RHLD balance, and in that same transaction the token contract sent them a pro-rata share of every bStock the treasury held at the time — CRCLB, NVDAB, SNDKB and MUB all moved from the treasury address to the burner's wallet, cited directly from that transaction's logs, not described ahead of proof. This site's Redeem button calls the same burn(uint256) function."
      ),
      p(
  ...
  },
  {
    ...
    blocks: [
      p(
        "Any wallet holding at least 100,000 RHLD can cast a non-binding, on-chain vote for which blue-chip stock the reserve should prioritize once Binance issues a bStock for it. Voting is gated by balance at the time of the vote, not by staking or locking tokens — you keep full custody and control throughout."
      ),
      p(
        "This is a signal, not a binding instruction: the governance contract has no access to RHLD or the treasury, and can't cause anything to actually be bought. It only reads RHLD's balance to decide who's eligible to vote."
      ),
    ],
  },
  {
  ...
  },
  {
    ...
    blocks: [
      ul([
        ...
        "RHLD represents a claim on a reserve of tokenized-equity exposure. This page is informational only, not financial or legal advice, and nothing here is an offer to sell securities.",
      ]),
    ],
  },
];
...
const zh: DocsSection[] = [
  {
    ...
    blocks: [
      p(
        "Reserve Holdings（RHLD）是运行于 BNB Chain 的固定总量 BEP-20 代币。启动时，代币合约本身直接开设了 PancakeSwap V3 交易池，并注入了全部初始流动性 —— 既不是部署者钱包，也不是锁仓服务，更没有通过头寸管理合约生成可被转让或出售的 NFT。本节及下一节的每一项陈述均已对照启动交易（交易哈希 0x9444fa2a…d44355）及交易池自身的事件日志核实。"
      ),
      p(
  ...
  },
  {
    ...
    blocks: [
      p(
        "RHLD 的全部供应量 —— 1,000,000,000 枚 —— 在启动交易中一次性直接铸造进入 PancakeSwap V3 交易池。在此之前及之后均未再发生任何铸造。"
      ),
      ul([
        "完全不收取转账税。交易成本仅为池子自身的手续费 —— 池子的 PoolCreated 事件确认费率为 100（PancakeSwap V3 单位，等于 0.01%）—— 并非代币层面额外收取的费用。",
        "对自己的余额调用 burn(uint256) 所做的不只是从总供应量中移除 RHLD：合约会在同一笔交易中，按比例将资金库持有的每一支 bStock 支付给您 —— 已由一笔真实执行的销毁交易确认（交易哈希 0xaa57e40d…ead9fb）。该交易的具体内容详见下方“资金库”部分。",
      ]),
    ],
  },
  {
    ...
    blocks: [
      p(
        "典型的做法是通过 PancakeSwap 的头寸管理合约（NonfungiblePositionManager）铸造头寸，并返回一枚 NFT —— 持有该 NFT 者即可移除流动性，而 NFT 和其他资产一样，可能被转让、遗失或盗取。RHLD 绕开了这一路径：代币合约直接调用了交易池自身的 mint() 函数，将自己指定为头寸的所有者，而非经由头寸管理合约。这一点已由启动交易中池子自身的 Mint 事件直接确认 —— owner 字段记录的正是 RHLD 代币合约地址，而非某个钱包、锁仓合约或 NFT。"
      ),
      p(
        "该头寸以单边方式注入 —— 仅有 RHLD，不含 BNB —— 区间从略高于启动价格一直延伸到 PancakeSwap V3 允许的最大刻度（tick）。买入带来的 BNB 会以交易手续费的形式累积，而非闲置在头寸之中。"
      ),
      p(
  ...
  },
  {
    ...
    blocks: [
      p(
        "资金库收购的 bStocks 直接持有在 RHLD 自身的合约地址上 —— 不存在单独的金库合约。这意味着任何人都可以随时在该地址的 BscScan “Tokens” 标签页中核实具体持有了什么、持有多少，而无需信任某个声称的数字。"
      ),
      p(
        "设计如下：每一笔针对 RHLD 交易池的交易都会产生一笔交易手续费，代币合约会以 BNB 形式收取该手续费（已确认实时自动发生，自第一笔交易起即是如此 —— 详见上文“流动性”部分），而这部分 BNB 正是用于购入一篮子 bStocks 的资金来源，购入后落地于同一个任何人都已可核实的资金库地址。手续费进来，bStocks 出去，全程都在同一个合约内完成 —— 中间没有单独的 keeper 钱包或转换合约经手资金。"
      ),
      p(
      ...
      p(
        "赎回功能已确认上线。在交易 0xaa57e40d…ead9fb 中，一位持有者对自己的 RHLD 余额调用了 burn(185,458.208224…)，而在同一笔交易中，代币合约按比例向其支付了当时资金库持有的每一支 bStock —— CRCLB、NVDAB、SNDKB 与 MUB 均从资金库地址转移到了该销毁者的钱包，这些数据直接引用自该交易的日志，而非在得到证实之前先行描述。本网站的“赎回”按钮调用的正是同一个 burn(uint256) 函数。"
      ),
      p(
  ...
  },
  {
    ...
    blocks: [
      p(
        "任何持有至少 100,000 RHLD 的钱包都可以为币安尚未发行 bStock 的蓝筹股投出非约束性的链上投票，表达储备下一步应优先收购哪一支。投票资格根据投票时的持仓判定，代币无需质押或锁定 —— 您始终完全掌控和拥有自己的代币。"
      ),
      p("这只是一种信号，而非约束性指令：治理合约无法访问 RHLD 或资金库，也无法促成任何实际购买。它仅读取 RHLD 余额以判断谁具备投票资格。"),
    ],
  },
  {
    id: "security",
    ...
    blocks: [
      p(
        "已在链上直接核实、并在上文引用具体交易作为依据的内容：流动性头寸由代币合约自身地址持有，而非钱包或 NFT；手续费以 BNB 形式自动收取，已在启动后第一笔交易中得到确认；全部供应量已在启动时一次性铸造，直接进入交易池；销毁 RHLD 会按比例向销毁者支付资金库持有的 bStock，已由一笔真实执行的销毁交易确认。不过请先参考上文“可升级性”部分：以上每一项描述的都是代币代理合约当前所指向实现合约的行为，而非永久性保证 —— 直到升级权限被放弃为止。"
      ),
      p(
  ...
  },
  {
    ...
    blocks: [
      ul([
        ...
        "RHLD 代表对一篮子代币化股票敞口储备的权益凭证。本页面仅供参考，不构成财务或法律建议，页面中的任何内容均不构成证券要约。",
      ]),
    ],
  },
];
