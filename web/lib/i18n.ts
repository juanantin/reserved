...
export const dictionaries = {
  en: {
    ...
    floor: {
      ...
      body: "RSVD is a fixed-supply token backed by a reserve of real, tokenized equities held directly on the token contract. Every holding is checkable on-chain, block by block — and any holder can burn RSVD to redeem a pro-rata share of it, confirmed by a real executed transaction (see the docs).",
    },
    dashboardCard: {
  ...
  zh: {
    ...
    floor: {
      ...
      headlineGold: "储备即是底线。",
      body: "RSVD 是固定总量的代币，背后由直接持有在代币合约地址上的真实代币化股票储备支撑。每一笔持仓均可逐块在链上核实 —— 任何持有者均可销毁 RSVD 按比例赎回相应份额，这一点已由一笔真实执行的交易确认（详见文档），而不仅仅是说明。",
    },
    dashboardCard: {
      marketCap: "市值",
      noPoolYet: "尚无交易池",
    ...
    transparency: {
      ...
      features: [
        ...
        {
          title: "销毁即可赎回，已链上确认",
          description: "销毁 RSVD 会在同一笔交易中按比例支付资金库持有的每一支 bStock —— 已由一笔真实执行的销毁交易确认（详见文档）。是否还存在其他仅所有者可用的提取途径，尚未通过源码独立核实；具体已核实内容详见文档中的“安全性”部分。",
        },
      ],
